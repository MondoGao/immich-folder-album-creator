...
ALBUM_THUMBNAIL_SETTINGS = [
    "first",
    "last",
    "random",
    ALBUM_THUMBNAIL_RANDOM_ALL,
    ALBUM_THUMBNAIL_RANDOM_FILTERED,
]
...
parser.add_argument(
    "root_path", action="append", help="The external libarary's root path in Immich"
)
parser.add_argument(
    "api_url", help="The root API URL of immich, e.g. https://immich.mydomain.com/api/"
)
...
parser.add_argument(
    ...
    "--root-path",
    action="append",
    help="Additional external libarary root path in Immich; May be specified multiple times for multiple import paths or external libraries.",
)
parser.add_argument(
    "-u",
    "--unattended",
...
parser.add_argument(
    ...
    "--insecure",
    action="store_true",
    help="Set to true to ignore SSL verification",
)
parser.add_argument(
    "-i",
    "--ignore",
    action="append",
    help="Use either literals or glob-like patterns to ignore assets for album name creation. This filter is evaluated after any values passed with --path-filter. May be specified multiple times.",
)
parser.add_argument(
    "-m",
    "--mode",
    default=SCRIPT_MODE_IMPORT,
    choices=[
        SCRIPT_MODE_CREATE,
        SCRIPT_MODE_CLEANUP,
        SCRIPT_MODE_DELETE_ALL,
        SCRIPT_MODE_IMPORT,
    ],
    help="Mode for the script to run with. CREATE = Create albums based on folder names and provided arguments; CLEANUP = Create album nmaes based on current images and script arguments, but delete albums if they exist; DELETE_ALL = Delete all albums. If the mode is anything but CREATE, --unattended does not have any effect. Only performs deletion if -d/--delete-confirm option is set, otherwise only performs a dry-run.",
)
parser.add_argument(
    "-d",
    "--delete-confirm",
    action="store_true",
    help="Confirm deletion of albums when running in mode "
    + SCRIPT_MODE_CLEANUP
    + " or "
    + SCRIPT_MODE_DELETE_ALL
    + ". If this flag is not set, these modes will perform a dry run only. Has no effect in mode "
    + SCRIPT_MODE_CREATE,
)
parser.add_argument(
    "-x",
    "--share-with",
    action="append",
    help="A user name (or email address of an existing user) to share newly created albums with. Sharing only happens if the album was actually created, not if new assets were added to an existing album. If the the share role should be specified by user, the format <userName>=<shareRole> must be used, where <shareRole> must be one of 'viewer' or 'editor'. May be specified multiple times to share albums with more than one user.",
)
parser.add_argument(
    "-o",
    "--share-role",
    ...
    help="The default share role for users newly created albums are shared with. Only effective if --share-with is specified at least once and the share role is not specified within --share-with.",
)
parser.add_argument(
    "-S",
    "--sync-mode",
    ...
    type=int,
    choices=[0, 1, 2],
    help="Synchronization mode to use. Synchronization mode helps synchronizing changes in external libraries structures to Immich after albums have already been created. Possible Modes: 0 = do nothing; 1 = Delete any empty albums; 2 = Delete offline assets AND any empty albums",
)
parser.add_argument(
    "-O",
    "--album-order",
...
parser.add_argument(
    ...
    action="store_true",
    help="By default, the script only finds assets that are not assigned to any album yet. Set this option to make the script discover assets that are already part of an album and handle them as usual. If --find-archived-assets is set as well, both options apply.",
)
parser.add_argument(
    "-f",
    "--path-filter",
    action="append",
    help="Use either literals or glob-like patterns to filter assets before album name creation. This filter is evaluated before any values passed with --ignore. May be specified multiple times.",
)
parser.add_argument(
    "--set-album-thumbnail",
    choices=ALBUM_THUMBNAIL_SETTINGS,
    help="Set first/last/random image as thumbnail for newly created albums or albums assets have been added to. If set to "
    + ALBUM_THUMBNAIL_RANDOM_FILTERED
    + ", thumbnails are shuffled for all albums whose assets would not be filtered out or ignored by the ignore or path-filter options, even if no assets were added during the run. If set to "
    + ALBUM_THUMBNAIL_RANDOM_ALL
    + ", the thumbnails for ALL albums will be shuffled on every run.",
)
parser.add_argument(
    "-v",
    "--archive",
    action="store_true",
    help="Set this option to automatically archive all assets that were newly added to albums. If this option is set in combination with --mode = CLEANUP or DELETE_ALL, archived images of deleted albums will be unarchived. Archiving hides the assets from Immich's timeline.",
)
parser.add_argument(
    "--find-archived-assets",
    action="store_true",
    help="By default, the script only finds assets that are not archived in Immich. Set this option to make the script discover assets that are already archived. If -A/--find-assets-in-albums is set as well, both options apply.",
)
parser.add_argument(
    "--synology-photo-import",
    help="csv with header (id, albumname, id_item, filename, dirname, path) that exported from synology photo database.",
)
...
synology_photo_import = args["synology_photo_import"]
...
logging.debug(
    "number_of_assets_to_fetch_per_request = %d", number_of_assets_to_fetch_per_request
)
...
def create_album_name(path_chunks: list[str], album_separator: str) -> str:
    ...
    if len(album_levels_range_arr) == 2:
        if album_levels_range_arr[0] < 0:
            album_levels_start_level_capped = min(
                len(path_chunks), abs(album_levels_range_arr[0])
            )
            album_levels_end_level_capped = album_levels_range_arr[1] + 1
            album_levels_start_level_capped *= -1
        else:
            album_levels_start_level_capped = min(
                len(path_chunks) - 1, album_levels_range_arr[0]
            )
            ...
            album_levels_end_level_capped = (
                min(len(path_chunks) - 1, album_levels_range_arr[1]) + 1
            )
        logging.debug(
            "album_levels_start_level_capped = %d", album_levels_start_level_capped
        )
        logging.debug(
            "album_levels_end_level_capped = %d", album_levels_end_level_capped
        )
        ...
        if album_levels_start_level_capped is not album_levels_end_level_capped:
            ...
            # if the end index is out of bounds.
            if album_levels_end_level_capped < 0 and abs(
                album_levels_end_level_capped
            ) >= len(path_chunks):
                album_name_chunks = path_chunks[album_levels_start_level_capped:]
            else:
                album_name_chunks = path_chunks[
                    album_levels_start_level_capped:album_levels_end_level_capped
                ]
...
def fetchServerVersion() -> dict:
    ...
    # Any other errors mean communication error with API
        ...
        logging.error(
            "Communication with Immich API failed! Make sure the passed API URL is correct!"
        )
        checkApiResponse(r)
    return version
...
def fetchAssets(isNotInAlbum: bool, findArchived: bool) -> list:
    ...
    assets = fetchAssetsWithOptions(
        {"isNotInAlbum": isNotInAlbum, "withArchived": findArchived}
    )
    return assets
...
def fetchAssetsWithOptions(searchOptions: dict) -> list:
    ...
    number_of_assets_to_fetch_per_request_search = min(
        1000, number_of_assets_to_fetch_per_request
    )
...
def createAlbum(albumName: str, albumOrder: str) -> str:
    ...
    data = {"albumName": albumName, "description": albumName}
    ...
    if albumOrder:
        ...
        r = requests.patch(
            root_url + apiEndpoint + f"/{albumId}", json=data, **requests_kwargs
        )
        checkApiResponse(r)
    return albumId
    ...
    if asset_root_path:
        ...
        if len(path_filter_regex) > 0:
            ...
            for path_filter_regex_entry in path_filter_regex:
                if re.fullmatch(
                    path_filter_regex_entry, asset_path.replace(asset_root_path, "")
                ):
                    any_match = True
            if not any_match:
                logging.debug(
                    "Ignoring asset %s due to path_filter setting!", asset_path
                )
        ...
        if not is_asset_ignored and len(ignore_albums_regex) > 0:
            for ignore_albums_regex_entry in ignore_albums_regex:
                if re.fullmatch(
                    ignore_albums_regex_entry, asset_path.replace(asset_root_path, "")
                ):
                    is_asset_ignored = True
                    logging.debug(
                        "Ignoring asset %s due to ignore_albums setting!", asset_path
                    )
                    break
...
def addAssetsToAlbum(albumId: str, assets: list[str]) -> list[str]:
    ...
    for assets_chunk in assets_chunked:
        ...
        r = requests.put(
            root_url + apiEndpoint + f"/{albumId}/assets", json=data, **requests_kwargs
        )
        checkApiResponse(r)
        response = r.json()
        ...
        for res in response:
            if not res["success"]:
                if res["error"] != "duplicate":
                    logging.warning(
                        "Error adding an asset to an album: %s", res["error"]
                    )
            else:
                cpt += 1
...
def shareAlbumWithUserAndRole(
    album_id: str, share_user_ids: list[str], share_role: str
):
    """
...
def triggerOfflineAssetRemovalSinceMinorVersion116():
    ...
    if len(offline_assets) > 0:
        logging.debug(
            "Deleting the following offline assets (count: %d): %s",
            len(offline_assets),
            [asset["originalPath"] for asset in offline_assets],
        )
...
def deleteAssets(assets_to_delete: list, force: bool):
    ...
    data = {"force": force, "ids": assetIdsToDelete}
...
def setAssetsArchived(assetIds: list[str], isArchived: bool):
    ...
    data = {"ids": assetIds, "isArchived": isArchived}
...
if not is_integer(album_levels):
    ...
    if (
        ...
        or (
            ...
            and int(album_levels_range_split[1]) < 0
        )
        or (
            int(album_levels_range_split[0]) < 0
            and int(album_levels_range_split[1]) >= 0
        ...
        and int(album_levels_range_split[0]) > int(album_levels_range_split[1])
    ):
        logging.error(
            "Invalid album_levels range format! If a range should be set, the start level and end level must be separated by a comma like '<startLevel>,<endLevel>'. If negative levels are used in a range, <startLevel> must be less than or equal to <endLevel>."
        )
        exit(1)
    ...
    if album_levels_range_arr[0] < 0 and album_levels_range_arr[1] == -1:
        ...
        album_levels_range_arr = ()
        logging.debug(
            "album_levels is a range with negative start level and end level of -1, converted to album_levels = %d",
            album_levels,
        )
    else:
...
if version["major"] == 1 and version["minor"] < 106:
    logging.fatal(
        "This script only works with Immich Server v1.106.0 and newer! Update Immich Server or use script version 0.8.1!"
    )
    exit(1)

if os.path.exists("assets.json"):
    logging.info("Loading assets cache")
...
if synology_photo_import is None:
    sys.exit(1)


path_to_asset = defaultdict()
for asset in assets:
    path = asset["originalPath"]
    if path in path_to_asset:
        logging.error(f"Duplicate asset found for path {path}")
        sys.exit(1)
    path_to_asset[path] = asset
synology_csv = csv.DictReader(open(synology_photo_import, "r"))
not_found_csv = csv.DictWriter(
    open("not_found.csv", "w"),
    fieldnames=synology_csv.fieldnames,
)

album_to_assets = defaultdict(list)
not_found = []
for row in synology_csv:
    albumname = row["albumname"]
    path = row["path"]
    found = False
    for root_path in root_paths:
        target_path = f"{root_path[:-1]}{path}"
        if target_path not in path_to_asset:
            continue
        found = True
        asset = path_to_asset[target_path]
        album_to_assets[albumname].append(asset["id"])
    if not found:
        not_found_csv.writerow(row)
        print(f"Asset not found for path {path}")
...
if share_with is not None and len(created_albums) > 0:
    ...
    for share_user in share_user_roles.keys():
        ...
        if role == None:
            role = share_role
            logging.debug(
                "No explicit share role passed for share user %s, using default role %s",
                share_user,
                share_role,
            )
            ...
            role = share_role
            logging.warning(
                "Passed share role %s for user %s is not allowed, defaulting to %s",
                role,
                share_user,
                share_role,
            )
            ...
            logging.debug(
                "Explicit share role %s passed for share user %s", role, share_user
            )
    ...
    for share_album in created_albums.keys():
        ...
        for role in roles_for_share_user_ids.keys():
            ...
            if len(share_user_ids) > 0:
                try:
                    shareAlbumWithUserAndRole(
                        created_albums[share_album], share_user_ids, role
                    )
                    logging.debug(
                        "Album %s shared with users IDs %s in role: %s)",
                        share_album,
                        share_user_ids,
                        role,
                    )
                    ...
                    logging.warning(
                        "Error sharing album %s for users %s in role %s",
                        share_album,
                        share_user_ids,
                        role,
                    )
        ...
        if album_shared_successfully:
            shared_album_cnt += 1
    logging.info(
        "Successfully shared %d/%d albums", shared_album_cnt, len(created_albums)
    )
...
if set_album_thumbnail:
    ...
    for album in albums:
        ...
        if len(assets) > 0:
            ...
            logging.info(
                "Using asset with index %d as thumbnail for album %s",
                index,
                album["albumName"],
            )
...
if sync_mode >= 1:
    ...
    if emptyAlbumCount > 0:
        logging.info(
            "Successfully deleted %d/%d empty albums!",
            deletedAlbumCount,
            emptyAlbumCount,
        )
    else:
